...
@Component({
  selector: 'app-rca-entry',
  standalone: true,
  imports: [CommonModule],
  template: `
    @if (loading()) {
      <div class="entry-state">Opening root cause analysis…</div>
    } @else if (empty()) {
      <div class="entry-empty">
        <h1 class="entry-title">Root Cause Analysis</h1>
        <p>No incidents are currently assigned to you for RCA.</p>
      </div>
    }
  `,
  styles: [`
...
export class RcaEntryComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.incidents.getAssignedIncidents().subscribe({
      next: list => {
        ...
        this.router.navigate(['/resolver/incidents', target.id, 'rca'], { replaceUrl: true });
      },
