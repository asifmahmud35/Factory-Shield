...
- Given multiple incidents exist, then I can filter by severity, department, status, or due date. 

- Given I open an incident, then I can view all incident details, attachments, timeline, and previous comments. 
...
- **trigger_event** (enum: ASSIGNED , OVERDUE , SLA_EXCEEDED , STATUS_CHANGED , EVIDENCE_UPLOADED , RESOLVED , ESCALATED , APPROVED , REJECTED ; required) — must 
...
- **root_cause_code:** EQUIPMENT_FAILURE , PROCEDURE_GAP , HUMAN_ERROR , DESIGN_DEFECT , 

- EXTERNAL_FACTOR , UNKNOWN_UNDER_INVESTIGATION 
